      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <Card>
          ...
          <div className="h-64">
            <ResponsiveContainer width="100%" height="100%">
              <PieChart>
                <Pie data={data.charts.riskDistribution} dataKey="value" nameKey="name" innerRadius={50} outerRadius={80} paddingAngle={4}>
            ...
            <tbody>
              ...
              <AnimatePresence initial={false}>
                {data.recentScans.map((scan) => (
                  ...
                  >
                    <td className="py-3 font-medium text-slate-200">{scan.scanType}</td>
                    <td className="py-3">{scan.result ? `${Math.round(scan.result.scamProbability)}%` : "-"}</td>
                    <td className="py-3">{scan.result ? <RiskBadge level={scan.result.trustRiskLevel} /> : "-"}</td>
                    <td className="py-3 text-slate-400">{new Date(scan.createdAt).toLocaleString()}</td>
                    <td className="py-3">
                      <div className="flex items-center justify-end gap-3">
                        <Link className="text-cyan-300 hover:underline" to={`/scans/${scan.id}`}>
                          Details
                        </Link>
                        <button
                          type="button"
                          aria-label={`Delete ${scan.scanType} scan`}
                          onClick={() => setDeleteTarget(scan)}
                          disabled={deletingId === scan.id}
                          className="rounded-lg border border-slate-700 bg-slate-900 p-2 text-slate-400 opacity-0 transition hover:border-rose-900 hover:bg-rose-950/50 hover:text-rose-200 group-hover:opacity-100 disabled:cursor-not-allowed disabled:opacity-50"
                        >
                          <TrashIcon />
                        </button>
                      </div>
        ...
        {deleteTarget && (
          ...
          >
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              exit={{ opacity: 0, y: 16 }}
              className="w-full max-w-md rounded-2xl border border-slate-800 bg-[#0b131d] p-5 shadow-card"
            >
              <p className="text-xs uppercase tracking-[0.18em] text-slate-500">Delete workflow</p>
              <h3 className="mt-2 font-display text-xl font-semibold text-slate-100">Remove this threat scan?</h3>
              <p className="mt-3 text-sm leading-6 text-slate-400">
                This will delete the stored scan result and remove it from recent workflows. The action cannot be undone.
              </p>
              <div className="mt-5 flex justify-end gap-3">
                ...
                <button
                  ...
                  className="rounded-xl border border-rose-900 bg-rose-950/70 px-4 py-2 text-sm font-medium text-rose-200 transition hover:bg-rose-900/80"
                >
                  Delete scan
                </button>
              </div>
...
function TrashIcon() {
  return (
    <svg viewBox="0 0 24 24" className="h-4 w-4" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
      ...
      <path d="M6.5 6l1 13a1.5 1.5 0 0 0 1.5 1.4h6a1.5 1.5 0 0 0 1.5-1.4l1-13" />
      <path d="M10 10.5v6" />
      <path d="M14 10.5v6" />
    </svg>
  );
